...
{
  try {
    assert_cellnotnull("Given c was null", c);
  } catch (runtime_error& e) {
    throw_error(e.what(), "cons.hpp::nonzerop(Cell*)");
  }
...
inline int get_int(Cell* const c)
{
  try {
    ...
    throw_error(e.what(), "cons.hpp::get_int(Cell*)");
  }
...
inline double get_double(Cell* const c)
{
  try {
    ...
    throw_error(e.what(), "cons.hpp::get_double(Cell*)");
  }
...
inline string get_symbol(Cell* const c)
{
  ...
  try {
    ...
    throw_error(e.what(), "cons.hpp::get_symbol(Cell*)");
  }
...
inline Cell* car(Cell* const c)
{
  try {
    ...
    throw_error(e.what(), "cons.hpp::car(Cell*)");
  }
...
inline Cell* cdr(Cell* const c)
{
  try {
    ...
    throw_error(e.what(), "cons.hpp::cdr(Cell*)");
  }
...
inline double get_value(Cell* const c)
{
  try {
    ...
    throw_error(e.what(), "cons.hpp::get_value(Cell*)");
  }
 ...
 */
inline Operation get_operation(string const opr)
{
  if (opr == "+") {
    return add_opr;
  } else if (opr == "if") {
...
inline Cell* get_formals(Cell* const c)
{
  try {
    ...
    throw_error(e.what(), "cons.hpp::get_formals(Cell*)");
  }
...
inline Cell* get_body(Cell* const c)
{
  try {
    ...
    throw_error(e.what(), "cons.hpp::get_body(Cell*)");
  }
  return c->get_body();
}
...
{
  try {
    assert_cellnotnull("Cannot evaluate null", c);
  } catch (runtime_error& e) {
    throw_error(e.what(), "cons.hpp::cell_eval(Cell*)");
  }
  return c->eval();
}

/**
 * \brief Calls a cell's apply() function.
 * \param c The cell needed to be applied.
 * \return The result from applying the cell.
 */
inline Cell* cell_apply(Cell* const c, Cell* const args)
{
  Cell* procedure = nil;
  try {
    assert_cellnotnull("Cannot apply null", c);
    
    // Extra evaluation to ensure c was not a symbol referring to the function.
    // Simple eval() of a procedure only returns a copy.
    procedure = c;
  } catch (runtime_error& e) {
    throw_error(e.what(), "cons.hpp::cell_apply(Cell*)");
  }
